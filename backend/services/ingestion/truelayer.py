...
class TrueLayerIngestionService:
    ...
    async def _fetch_transactions(
        ...
        for entity in entities:
            ...
            response.raise_for_status()
            transactions.extend(response.json().get("results", []))
        return transactions

    def _upsert_transaction(self, transaction: dict[str, Any], db: Session) -> FinanceData | None:
        ...
        record = db.scalar(select(FinanceData).where(FinanceData.transaction_id == transaction_id))
        if not record:
            record = FinanceData(transaction_id=transaction_id, date=tx_date, amount_pence=amount, category="other")
            db.add(record)
